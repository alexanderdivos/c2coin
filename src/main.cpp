...
const string strMessageMagic = "C2coin Signed Message:\n";
...
{
    // C2coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 210.000 blocks, which will occur approximately every 1 year
    nSubsidy >>= (nHeight / 210000); // C2coin: 210k blocks in ~1 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 60 * 60; // C2coin: 10 hours
static const int64 nTargetSpacing = 2.5 * 60; // C2coin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // C2coin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        int DiffMode = 1;
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 1320) { DiffMode = 3; }
        }
        else {
                if (pindexLast->nHeight+1 >= 6570) { DiffMode = 3; }
                else if (pindexLast->nHeight+1 >= 5554) { DiffMode = 3; }  //CB ???
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // C2coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // C2coin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        //CB
        hashGenesisBlock = uint256("0xd3eff7bddab016c9e23f08bee9fdff8cbc28cc40217255205afcceb33063fe3b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        //CB
        assert(block.hashMerkleRoot == uint256("0xfd70f0e7f46416461181587bf5e14f7cdf0796b21c87fbef4cd245e1355b4556"));

        //CB Code from http://pastebin.com/jDMPUNuP#
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // C2coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("C2coinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("C2coinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static C2coinMiner(CWallet *pwallet)
{
    printf("C2coinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("c2coin-miner");
    ...
    try { loop {
        ...
        printf("Running C2coinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&C2coinMiner, pwallet));
}
